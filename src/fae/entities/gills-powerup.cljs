...
(defn handle-bumped [powerup state {bumpee :bumpee
  ...
  (let [bumper (entities/get-by-id bumper-id state)]
    (if (and (= :player (:type bumper))
             (= bumpee (:id powerup)))
      (do (events/trigger-event! :powerup-get {:kind :gills})
          (events/trigger-event! :log-entry-posted {:msg "You feel your gills tingle."})
          (events/trigger-event! :log-entry-posted {:msg "You can now move through water."})
