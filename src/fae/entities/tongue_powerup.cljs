(ns fae.entities.tongue-powerup
  (:require
...
(defn handle-bumped [powerup state {bumpee :bumpee
  ...
  (let [bumper (entities/get-by-id bumper-id state)]
    (if (and (= :player (:type bumper))
             (= bumpee (:id powerup)))
      (do (events/trigger-event! :powerup-get {:kind :tongue-length})
          (events/trigger-event! :log-entry-posted {:msg "Your tongue feels stretchy."})
          (events/trigger-event! :log-entry-posted {:msg "Lick range increased."})
